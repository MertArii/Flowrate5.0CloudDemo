...
-- public.sap_modules definition

-- Drop table

-- DROP TABLE public.sap_modules;

CREATE TABLE public.sap_modules ( id uuid DEFAULT uuid_generate_v4() NOT NULL, code text NOT NULL, created_at timestamptz DEFAULT CURRENT_TIMESTAMP NULL, CONSTRAINT sap_modules_code_key UNIQUE (code), CONSTRAINT sap_modules_pkey PRIMARY KEY (id));

-- Permissions

ALTER TABLE public.sap_modules OWNER TO helpdesk;
GRANT ALL ON TABLE public.sap_modules TO helpdesk;
...
-- public.tmp_duzeltilecek_routing_logs definition

-- Drop table

-- DROP TABLE public.tmp_duzeltilecek_routing_logs;

CREATE TABLE public.tmp_duzeltilecek_routing_logs ( id uuid NULL, ticket_id uuid NULL, decision_factors jsonb NULL, assigned_group_id uuid NULL, assigned_agent_id uuid NULL, confidence_score float8 NULL, is_overridden_by_human bool NULL, correct_group_id uuid NULL, created_at timestamptz NULL, assigned_agent_name text NULL, subject text NULL, raw_issue_description text NULL);

-- Permissions

ALTER TABLE public.tmp_duzeltilecek_routing_logs OWNER TO helpdesk;
GRANT ALL ON TABLE public.tmp_duzeltilecek_routing_logs TO helpdesk;


-- public.ust_kategoriler definition

-- Drop table

-- DROP TABLE public.ust_kategoriler;

CREATE TABLE public.ust_kategoriler ( id uuid DEFAULT uuid_generate_v4() NOT NULL, "name" text NOT NULL, is_active bool DEFAULT true NULL, created_at timestamptz DEFAULT CURRENT_TIMESTAMP NULL, CONSTRAINT ust_kategoriler_name_key UNIQUE (name), CONSTRAINT ust_kategoriler_pkey PRIMARY KEY (id));

-- Permissions

ALTER TABLE public.ust_kategoriler OWNER TO helpdesk;
GRANT ALL ON TABLE public.ust_kategoriler TO helpdesk;


-- public.kategori_gruplari definition

-- Drop table

-- DROP TABLE public.kategori_gruplari;

CREATE TABLE public.kategori_gruplari ( id uuid DEFAULT uuid_generate_v4() NOT NULL, ust_kategori_id uuid NOT NULL, "name" text NOT NULL, is_active bool DEFAULT true NULL, created_at timestamptz DEFAULT CURRENT_TIMESTAMP NULL, CONSTRAINT kategori_gruplari_pkey PRIMARY KEY (id), CONSTRAINT kategori_gruplari_ust_kategori_id_name_key UNIQUE (ust_kategori_id, name), CONSTRAINT kategori_gruplari_ust_kategori_id_fkey FOREIGN KEY (ust_kategori_id) REFERENCES public.ust_kategoriler(id) ON DELETE CASCADE);
CREATE INDEX idx_kategori_gruplari_ust ON public.kategori_gruplari USING btree (ust_kategori_id);

-- Permissions

ALTER TABLE public.kategori_gruplari OWNER TO helpdesk;
GRANT ALL ON TABLE public.kategori_gruplari TO helpdesk;
...
-- public.alt_kategoriler definition

-- Drop table

-- DROP TABLE public.alt_kategoriler;

CREATE TABLE public.alt_kategoriler ( id uuid DEFAULT uuid_generate_v4() NOT NULL, grup_id uuid NOT NULL, "name" text NOT NULL, is_active bool DEFAULT true NULL, created_at timestamptz DEFAULT CURRENT_TIMESTAMP NULL, CONSTRAINT alt_kategoriler_grup_id_name_key UNIQUE (grup_id, name), CONSTRAINT alt_kategoriler_pkey PRIMARY KEY (id), CONSTRAINT alt_kategoriler_grup_id_fkey FOREIGN KEY (grup_id) REFERENCES public.kategori_gruplari(id) ON DELETE CASCADE);
CREATE INDEX idx_alt_kategoriler_grup ON public.alt_kategoriler USING btree (grup_id);

-- Permissions

ALTER TABLE public.alt_kategoriler OWNER TO helpdesk;
GRANT ALL ON TABLE public.alt_kategoriler TO helpdesk;
...
CREATE TABLE public.tickets ( id uuid DEFAULT uuid_generate_v4() NOT NULL, ticket_number serial4 NOT NULL, customer_email varchar(150) NOT NULL, customer_id uuid NULL, recipient_email varchar(150) NOT NULL, subject varchar(255) NOT NULL, raw_issue_description text NOT NULL, extracted_category varchar(100) NULL, region varchar(100) NULL, status varchar(30) DEFAULT 'new'::character varying NULL, priority varchar(20) DEFAULT 'medium'::character varying NULL, assigned_group_id uuid NULL, assigned_agent_id uuid NULL, created_at timestamptz DEFAULT CURRENT_TIMESTAMP NULL, updated_at timestamptz DEFAULT CURRENT_TIMESTAMP NULL, resolved_at timestamptz NULL, sla_policy_id uuid NULL, response_deadline timestamptz NULL, workaround_deadline timestamptz NULL, resolution_deadline timestamptz NULL, first_response_at timestamptz NULL, sla_status varchar(20) DEFAULT 'within_sla'::character varying NULL, last_paused_at timestamptz NULL, total_paused_duration interval DEFAULT '00:00:00'::interval NULL, sub_category_id uuid NULL, sap_module_id uuid NULL, CONSTRAINT tickets_pkey PRIMARY KEY (id), CONSTRAINT tickets_priority_check CHECK (((priority)::text = ANY (ARRAY['low'::text, 'medium'::text, 'high'::text, 'urgent'::text, 'planned'::text]))), CONSTRAINT tickets_status_check CHECK (((status)::text = ANY (ARRAY['new'::text, 'l1_routing'::text, 'assigned'::text, 'in_progress'::text, 'waiting'::text, 'resolved'::text, 'closed'::text]))), CONSTRAINT tickets_ticket_number_key UNIQUE (ticket_number), CONSTRAINT tickets_assigned_agent_id_fkey FOREIGN KEY (assigned_agent_id) REFERENCES public.users(id) ON DELETE SET NULL, CONSTRAINT tickets_assigned_group_id_fkey FOREIGN KEY (assigned_group_id) REFERENCES public.support_groups(id) ON DELETE SET NULL, CONSTRAINT tickets_customer_id_fkey FOREIGN KEY (customer_id) REFERENCES public.users(id) ON DELETE SET NULL, CONSTRAINT tickets_sap_module_id_fkey FOREIGN KEY (sap_module_id) REFERENCES public.sap_modules(id) ON DELETE SET NULL, CONSTRAINT tickets_sla_policy_id_fkey FOREIGN KEY (sla_policy_id) REFERENCES public.sla_policies(id), CONSTRAINT tickets_sub_category_id_fkey FOREIGN KEY (sub_category_id) REFERENCES public.alt_kategoriler(id) ON DELETE SET NULL);
